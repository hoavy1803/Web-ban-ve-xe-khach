...
templatesManager = Jinja2Templates(
    directory="D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\manager\\templates"
)
...
app.mount(
    "/img-m",
    StaticFiles(
        directory="D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\img"
    ),
    name="img-m",
)
app.mount(
    "/css-m",
    StaticFiles(
        directory="D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\manager\\css-m"
    ),
    name="css-m",
)
app.mount(
    "/js-m",
    StaticFiles(
        directory="D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\manager\\js-m"
    ),
    name="js-m",
)
...
def get_khachhang(request: Request):
    ...
    cursor.execute(
        "SELECT maKhachHang, tenKhachHang, email, soDienThoai_KH, soTaiKhoanNH FROM khachhang "
    )
    data = cursor.fetchall()  # Trả về tất các các kết quả truy vấn
    cursor.close()
    return templatesManager.TemplateResponse(
        "passenger.html", {"request": request, "data": data}
    )
...
async def edit_khachhang_form(request: Request):
    return templatesManager.TemplateResponse(
        "edit_khachhang.html", {"request": request}
    )
...
def get_khachhang(request: Request):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT maNhanVien, tenNhanVien, chucVu, gioiTinh, soDienThoai_NV, email, ngaySinh, diaChi, luong FROM nhanvien "
    )
...
#     return templatesManager.TemplateResponse(
#         "delete_nhanvien.html", {"request": request}
#     )
...
def get_datve(request: Request):
    cursor = conn.cursor()
    cursor.execute("SELECT datve.*, gioDi, giaChuyenDi FROM datve, chuyendi WHERE datve.maChuyenDi = chuyendi.maChuyenDi")
    data = cursor.fetchall()
    cursor.close()
    return templatesManager.TemplateResponse(
        "ticket.html", {"request": request, "data": data}
    )
...
# Edit bảng datve
@app.get("/edit_datve/{id}", response_class=HTMLResponse)
...
async def edit_datve(
    ...
    try:
        cursor.execute(
            "UPDATE datve SET soLuong = %s, maChuyenDi = %s, maKhachHang = %s, maThanhToan = %s WHERE maVe = %s",
            (soLuong, maChuyenDi, maKhachHang, maThanhToan, maVe),
        )
        conn.commit()
...
@app.get("/delete_datve/{id}", response_class=HTMLResponse)
...
def get_chuyendi(request: Request):
    ...
    cursor.close()
    return templatesManager.TemplateResponse(
        "trip.html", {"request": request, "data": data}
    )
...
# Edit bảng chuyendi
@app.get("/edit_chuyendi/{id}", response_class=HTMLResponse)
...
async def edit_chuyendi(
    ...
    try:
        cursor.execute(
            "UPDATE chuyendi SET soGheDat = %s, gioDi = %s, gioDen = %s, giaChuyenDi = %s, taiXe = %, maXeKhach = %s, maLoTrinh = %s WHERE maChuyenDi = %s",
            (
                soGheDat,
                gioDi,
                gioDen,
...
@app.get("/delete_chuyendi/{id}", response_class=HTMLResponse)
async def delete_chuyendi(request: Request):
    return templatesManager.TemplateResponse(
        "delete_chuyendi.html", {"request": request}
    )
...
def get_xekhach(request: Request):
    ...
    cursor.close()
    return templatesManager.TemplateResponse(
        "bus.html", {"request": request, "data": data}
    )
...
# Edit bảng xekhach
@app.get("/edit_xekhach/{id}", response_class=HTMLResponse)
...
async def edit_xekhach(
    ...
    try:
        cursor.execute(
            "UPDATE xekhach SET loaiGhe = %s, soGhe = %s, phuPhi = %s WHERE maXeKhach = %s",
            (loaiGhe, soGhe, phuPhi, maXeKhach),
        )
        conn.commit()
...
@app.get("/delete_xekhach/{id}", response_class=HTMLResponse)
async def delete_xekhach(request: Request):
    return templatesManager.TemplateResponse(
        "delete_xekhach.html", {"request": request}
    )
...
def get_lotrinh(request: Request):
    ...
    cursor.close()
    return templatesManager.TemplateResponse(
        "routes.html", {"request": request, "data": data}
    )
...
async def add_lotrinh(
    benXuatPhat: str = Form(...),
    benKetThuc: str = Form(...),
    tinhXuatPhat: str = Form(...),
    tinhKetThuc: str = Form(...),
    giaLoTrinh: float = Form(...),
    thoiGianDiChuyen: str = Form(...),
):
    cursor = conn.cursor()
    try:
        cursor.execute(
            "INSERT INTO lotrinh (benXuatPhat, benKetThuc, tinhXuatPhat, tinhKetThuc, giaLoTrinh, thoi_gian_di_chuyen) VALUES (%s, %s, %s, %s, %s, %s)",
            (benXuatPhat, benKetThuc, tinhXuatPhat, tinhKetThuc, giaLoTrinh, thoiGianDiChuyen),
        )
        conn.commit()
...
# Edit bảng lotrinh
@app.get("/edit_lotrinh/{id}", response_class=HTMLResponse)
...
async def edit_lotrinh(
    ...
    tinhKetThuc: str = Form(...),
    giaLoTrinh: float = Form(...),
    thoi_gian_di_chuyen: str = Form(...),
):
    cursor = conn.cursor()
    try:
        cursor.execute(
            "UPDATE lotrinh SET benXuatPhat = %s, benKetThuc = %s, tinhXuatPhat = %s, tinhKetThuc = %s, giaLoTrinh = %s, thoi_gian_di_chuyen = %s WHERE maLoTrinh = %s",
            (benXuatPhat, benKetThuc, tinhXuatPhat, tinhKetThuc, giaLoTrinh, thoi_gian_di_chuyen, maLoTrinh),
        )
        conn.commit()
...
@app.get("/delete_lotrinh/{id}", response_class=HTMLResponse)
async def delete_lotrinh(request: Request):
    return templatesManager.TemplateResponse(
        "delete_lotrinh.html", {"request": request}
    )


@app.post("/delete_lotrinh")
async def delete_lotrinh_post(request: Request, maLoTrinh: int = Form(...)):
    cursor = conn.cursor()
    try:
...
def get_congviec(request: Request):
    ...
    cursor.close()
    return templatesManager.TemplateResponse(
        "payment.html", {"request": request, "data": data}
    )


@app.post("/add_thanhtoan")
async def add_thanhtoan(trangThai: int = Form(...), tongTien: float = Form(...)):
    cursor = conn.cursor()
    try:
        cursor.execute(
...
# Edit bảng thanhtoan
@app.get("/edit_thanhtoan/{id}", response_class=HTMLResponse)
async def edit_thanhtoan_form(request: Request):
    return templatesManager.TemplateResponse(
        "edit_thanhtoan.html", {"request": request}
    )
...
async def edit_thanhtoan(
    ...
    try:
        cursor.execute(
            "UPDATE thanhtoan SET trangThai = %s, tongTien = %s WHERE maThanhToan = %s",
            (trangThai, tongTien, maThanhToan),
        )
        conn.commit()
...
@app.get("/delete_thanhtoan/{id}", response_class=HTMLResponse)
async def delete_thanhtoan(request: Request):
    return templatesManager.TemplateResponse(
        "delete_thanhtoan.html", {"request": request}
    )


@app.post("/delete_thanhtoan")
async def delete_thanhtoan_post(request: Request, maThanhToan: int = Form(...)):
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM thanhtoan WHERE maThanhToan = %s", (maThanhToan,))
        conn.commit()
        return {"message": "Deleted successfully"}
    except mysql.connector.Error as e:
        conn.rollback()
...
@app.post("/update_discount")
def update_discount(discount_update: DiscountUpdate):
    cursor = conn.cursor()
    current_discount = discount_update.discount
    select_day = discount_update.selectedDay
    cursor.execute(f"SELECT maChuyenDi FROM chuyendi WHERE SUBSTRING(chuyendi.gioDi, 1, 10) = '{select_day}'")
    id_trip=cursor.fetchall()
    for i in id_trip:
        cursor.execute(f"SELECT giaChuyenDi FROM chuyenDi WHERE maChuyenDi = '{i[0]}'")
        fare_trip=cursor.fetchone()[0]*(1-current_discount*0.01)
        cursor.execute(f"UPDATE chuyendi SET giaChuyenDi = '{fare_trip}' WHERE maChuyenDi = '{i[0]}'")
        conn.commit()
    return {"message": "Cập nhật giảm giá thành công", "new_discount": current_discount}