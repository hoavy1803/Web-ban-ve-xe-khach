...
templates = Jinja2Templates(
    directory="D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\user\\templates"
)
# Kết nối thư mục "css" ,"js" và "img" để phục vụ các tệp tin tĩnh

# Đường dẫn tuyệt đối đến thư mục img
img_directory = (
    "D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\user\\img-u"
)
...
app.mount(
    "/css-u",
    StaticFiles(
        directory="D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\user\\css-u"
    ),
    name="css-u",
)
app.mount(
    "/js-u",
    StaticFiles(
        directory="D:\\Năm 3\\CSDL web\\THỰC HÀNH WEB\\ban-ve-xe-khach\\frontend\\user\\js-u"
    ),
    name="js-u",
)
...
@app.post("/vehientai/me/{id_KH}")
def check_ticket(id_KH: int):
    cursor = conn.cursor()
    query = "SELECT * FROM chuyendi JOIN datve ON chuyendi.maChuyenDi = datve.machuyendi JOIN thanhtoan ON datve.maThanhToan = thanhtoan.maThanhToan WHERE datve.maKhachHang = %s;"
    ...
    for row in data:
        ticket = Ticket(
            tripId=row[0],
            slot=row[1],
            time1=row[2].strftime("%Y-%m-%d"),
            time2=row[3].strftime("%Y-%m-%d"),
            # price=row[4],
            # driver=row[5],
            # busId=row[6],
            routerId=row[7],
            ...
            userId=row[11],
            payId=row[12],
            status="Đã thanh toán" if row[14]==1 else "Chưa thanh toán",
            bill=row[15]
        )
        data_tickets.append(ticket)

    cursor.close()
    return {"data_ticket": data_tickets}
...
def get_lo_trinh():
    ...
    cursor.execute(
        "SELECT DISTINCT benXuatPhat, benKetThuc, tinhXuatPhat, tinhKetThuc FROM lotrinh"
    )
    data = cursor.fetchall()
...
try:
    ...
    cursor.execute(
        "INSERT INTO temp_lotrinhs (maLoTrinh) SELECT maLoTrinh FROM lotrinh"
    )
    # Lấy ngày hiện tại
    ...
    for _ in range(7):
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM chuyendi WHERE DATE(gioDi) = %s", (current_date,)
        )
        ...
        if count == 0:
            ...
            for row in cursor.fetchall():
                ...
                for gio_di in gio_di_list:
                    ...
                    cursor.execute(
                        "SELECT maXeKhach FROM xekhach WHERE loaiGhe='Giường nằm' ORDER BY RAND() LIMIT 1 "
                    )
                    maXeKhach = cursor.fetchone()[0]
                    # Lấy giá trị ma tai xe từ bảng xekhach
                    cursor.execute(
                        "SELECT maNhanVien FROM nhanvien WHERE chucVu='Tài xế' ORDER BY RAND() LIMIT 1 "
                    )
                    maTaiXe = cursor.fetchone()[0]
                    # Lấy thông tin thời gian di chuyển từ bảng lotrinh
                    cursor.execute(
                        "SELECT thoi_gian_di_chuyen FROM lotrinh WHERE maLoTrinh = %s",
                        (maLoTrinh,),
                    )
                    thoi_gian_di_chuyen = cursor.fetchone()[0]
                    # Lấy giá lộ trình từ bảng lotrinh và phụ phí từ bảng xe khách
                    cursor.execute(
                        "SELECT giaLoTrinh FROM lotrinh WHERE maLoTrinh=%s",
                        (maLoTrinh,),
                    )
                    giaLoTrinh = cursor.fetchone()[0]
                    cursor.execute(
                        "SELECT phuPhi FROM xekhach WHERE maXeKhach = %s ", (maXeKhach,)
                    )
                ...
                for gio_di in gio_di_list:
                    # Lấy giá trị maXeKhach từ bảng xekhach
                    cursor.execute(
                        "SELECT maXeKhach FROM xekhach WHERE loaiGhe='Ghế ngồi' ORDER BY RAND() LIMIT 1 "
                    )
                    maXeKhach = cursor.fetchone()[0]
                    # Lấy giá trị ma tai xe từ bảng xekhach
                    cursor.execute(
...
try:
    ...
    print(
        f"Đã xóa các chuyến đi trong ngày {current_date} mà số ghế đặt = 0 thành công."
    )
except Exception as e:
    # Rollback nếu có lỗi
    conn.rollback()
...
async def book_success(request_data: UpdateSeatRequest, userId: int):
    ...
    if chuyendi:
        ...
        car_id=chuyendi[6]
        id_route = chuyendi[7]
    cursor.execute(f"SELECT loaiGhe FROM xekhach WHERE maXeKhach='{car_id}'")
    type_bus=cursor.fetchone()[0]
    if type_bus=="Ghế ngồi":
    ...
    cursor.execute(f"SELECT xekhach.soGhe FROM xekhach JOIN chuyendi ON xekhach.maXeKhach = chuyendi.maXeKhach WHERE chuyendi.maChuyenDi = '{tripId}'")
    soGheBanDau= cursor.fetchone()[0]
    if ticketNumber > (soGheBanDau-soGheDaCo):

        cursor.execute("SELECT MAX(maChuyenDi) FROM chuyendi")
        old_machuyendi = cursor.fetchone()[0]
        tripId = old_machuyendi + 1
        new_trip_sql = f"""
                                    -- Thực hiện truy vấn để thêm chuyến xe mới vào cơ sở dữ liệu

                                    INSERT INTO chuyendi (maChuyenDi, soGheDat, gioDi, gioDen, giaChuyenDi,taiXe,maXeKhach,maLoTrinh)
                                    VALUES ('{tripId}','{ticketNumber}', '{gio_di}', '{gio_den}', '{fare}',0,'{car_id}','{id_route}');
                                """
    ...
    cursor.execute(
        "INSERT INTO datve (maVe,soLuong, maChuyenDi,maKhachHang,maThanhToan) VALUES (%s,%s,%s,%s,%s)",
